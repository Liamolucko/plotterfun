...
function getCircles(getPixel, rect, numDots, dotsPerPx) {
    ...
    if (numDots == 1) {
        // Find the average position in this rectangle weighted by each pixel's darkness.
        let totalDarkness = 0;
        let weightedSumX = 0;
        let weightedSumY = 0;
        ...
        for (let x = Math.floor(rect.left); x < rect.right; x++) {
            for (let y = Math.floor(rect.top); y < rect.bottom; y++) {
                ...
                weightedSumX += darkness * x;
                weightedSumY += darkness * y;
            }
        }

        let weightedAvgX = weightedSumX / totalDarkness;
        let weightedAvgY = weightedSumY / totalDarkness;
        return [
            [
                weightedAvgX,
                weightedAvgY,
